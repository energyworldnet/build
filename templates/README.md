...
Runs [pre-commit](https://pre-commit.com/) checks on pull requests. Supports optional .NET, pnpm, and Terraform toolchains.

### Parameters

| Parameter | Type | Default | Purpose |
|-----------|------|---------|---------|
| `pnpmCommands` | object | `[]` | Extra pnpm commands to run before pre-commit |
| `terraformVersion` | string | `1.12.1` | Terraform version to install |
| `useDotNet` | boolean | `false` | Restore .NET tools |
| `usePnpm` | boolean | `false` | Install and restore pnpm dependencies |
...
Step template that installs pnpm, authenticates against an internal npm feed, and restores dependencies.
...
Variable template that sets the `PnpmStorePath` variable used by `pnpm-tasks.yml`. Must always be included alongside `pnpm-tasks.yml`.
